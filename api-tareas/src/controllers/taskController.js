...
const getTasks = async (req, res, next) => {
  try {
    ...
    const myTasks = tasks.filter((task) => task.usuarioId === req.user.id);
...
const updateTask = async (req, res, next) => {
  try {
    ...
    const { titulo, descripcion } = req.body;
    const tasks = await readJsonFile(TASKS_FILE);
    const taskIndex = tasks.findIndex((task) => task.id === id && task.usuarioId === req.user.id);
...
const deleteTask = async (req, res, next) => {
  try {
    const { id } = req.params;
    const tasks = await readJsonFile(TASKS_FILE);
    const taskExists = tasks.some((task) => task.id === id && task.usuarioId === req.user.id);

    if (!taskExists) {
      return res.status(404).json({ message: 'Tarea no encontrada' });
    }

    const filteredTasks = tasks.filter((task) => !(task.id === id && task.usuarioId === req.user.id));
